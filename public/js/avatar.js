...
let isRecognitionActive = false; // 音声認識がアクティブかどうか
const RECOGNITION_LOCK_KEY = "speechRecognitionLock";
const LOCK_EXPIRATION_TIME = 5000; // ロックの有効期間（ミリ秒）
...
function acquireLock() {
  ...
  if (lockTime && currentTime - lockTime < LOCK_EXPIRATION_TIME) {
    console.log("[LOCK] 他のタブがロックを保持中");
    return false; // 他のタブがロックを保持中
  }

  localStorage.setItem(RECOGNITION_LOCK_KEY, currentTime);
  console.log("[LOCK] ロックを取得しました");
  return true;
}

function releaseLock() {
  localStorage.removeItem(RECOGNITION_LOCK_KEY);
  console.log("[LOCK] ロックを解放しました");
}
...
function initializeSpeechRecognition() {
  if (!("webkitSpeechRecognition" in window || "SpeechRecognition" in window)) {
    console.error("[SpeechRecognition] このブラウザはWeb Speech APIをサポートしていません");
  ...
  recognition.interimResults = true; // 暫定結果を取得
  ...
  recognition.onstart = () => {
    ...
    console.log("[SpeechRecognition] 音声認識が開始されました");
  };
  ...
  recognition.onerror = (event) => {
    ...
    if (event.error === "not-allowed") {
      console.log("[SpeechRecognition] ユーザーにより音声認識が拒否されました。");
      return; // 再試行を停止
    }
    handleRestartAfterError(); // エラー後の再起動処理
  };

  recognition.onend = () => {
    isRecognitionActive = false;
    console.log("[SpeechRecognition] 音声認識が終了しました");

    if (!isLocked()) {
      console.log("[SpeechRecognition] 他のタブがロックを保持しているため再起動をスキップします");
      return;
    }

    handleRestartAfterError(); // 正常終了時の再起動処理
  };
  ...
  if (acquireLock()) {
    try {
      ...
      console.error("[SpeechRecognition] 開始中にエラー:", error);
    }
  } else {
    console.warn("[SpeechRecognition] ロックが取得されているため、音声認識を開始しません");
  }
}
 ...
 * 音声認識の再起動を処理
 */
function handleRestartAfterError() {
  if (!isRecognitionActive && acquireLock()) {
    setTimeout(() => {
      try {
        console.log("[SpeechRecognition] 音声認識を再起動します...");
        recognition.start();
      } catch (error) {
        console.error("[SpeechRecognition] 再起動中にエラーが発生しました:", error);
      }
    }, 1000); // 再起動前の短い待機
  } else {
    console.log("[SpeechRecognition] 再起動中に他のタブがロックを取得しました");
  }
}
...
async function handleRecognitionResult(event) {
  ...
  console.log("[SpeechRecognition] 認識結果:", transcript);

  try {
    if (expressionMode === "rule") {
      updateAvatarExpressionFromSpeech(transcript);
    } else if (expressionMode === "gpt") {
      await updateAvatarExpressionFromGPT(transcript);
    }
  } catch (error) {
    console.error("[SpeechRecognition] 結果処理中にエラー:", error);
  }
}
 ...
 * ChatGPT APIを使った感情分類
 */
async function updateAvatarExpressionFromGPT(transcript) {
  try {
    const response = await fetch("http://127.0.0.1:3001/classify-emotion", {
      method: "POST",
    ...
    if (!response.ok) {
      throw new Error(`[ChatGPT API] エラー: ${response.statusText}`);
    }

    const { emotion } = await response.json();
    console.log("[ChatGPT API] 分類された感情:", emotion);

    const mappedEmotion = mapExpressionToAvatarEmotion(emotion);
    console.log("[Avatar] マッピング後の表情:", mappedEmotion);

    changeFace(mappedEmotion); // 表情を変更
    emitExpressionChange(mappedEmotion, "gpt"); // GPTを出所とする表情変更を通知
  } catch (error) {
...
function setupModeSwitchButton() {
  ...
  if (!toggleButton) {
    console.error("[ModeSwitch] モード切り替えボタンが見つかりません");
    return;
  }

  toggleButton.addEventListener("click", () => {
    expressionMode = expressionMode === "rule" ? "gpt" : "rule";
    toggleButton.textContent =
      expressionMode === "rule" ? "Switch to ChatGPT" : "Switch to Rule-Based";
    console.log(`[ModeSwitch] モードが切り替わりました: ${expressionMode}`);
  });
}
...
function updateAvatarExpressionFromSpeech(transcript) {
  ...
  console.log("[Avatar] ルールベースの表情:", matchedExpression);
  changeFace(matchedExpression);
}
